...
def save_keypoint_data(filename, camera, pose, keypoints, descriptors):
    kpdata =  np.array([(kp.pt[0], kp.pt[1], kp.size, kp.angle, kp.response, kp.octave, kp.class_id)
                        for kp in keypoints])
    np.savez(filename, camera=camera, pose=pose, keypoints=kpdata, descriptors=descriptors)
    return filename

def draw_str(dst, target, s):
    x, y = target
    cv2.putText(dst, s, (x+1, y+1), cv2.FONT_HERSHEY_PLAIN, 10.0, (0, 0, 0), thickness = 2, lineType=cv2.LINE_AA)
    cv2.putText(dst, s, (x, y), cv2.FONT_HERSHEY_PLAIN, 10.0, (255, 255, 255), lineType=cv2.LINE_AA)
...
def explore_features(config, filename, img, kp, des):
    ...
    if config.save:
        ...
        destname = filename[:filename.rfind('.')] + '-' + asift + config.feature
        if config.output:
...
def asift_query_feature(config):
    ...
    for filename in get_filenames(config.images):
...
def get_cells(h, w, row=3, col=3):
    rows = range(0, h + 1, h / row)
    rows[-1] = h - 1
    cols = range(0, w + 1, w / col)
    cols[-1] = w - 1
    cells = []
    for i in range(row):
        for j in range(col):
