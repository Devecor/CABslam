...
from config import kp_query_match_threshold, kp_feature_count
from feature import match_features
...
def query_image(config, refplane, image, camera):
    refDistance = refplane['pose'][0]
    refAzimuth = refplane['pose'][1]
    # 这是相机坐标系的顺序，即 Y 轴是朝下的
    refPos = np.float64(refplane['pose'][2:5])
    refCamera = refplane['camera']
    keypoints = refplane['keypoints']
    descriptors = refplane['descriptors']
    points3d = refplane.get('points3d', None)

    result = match_features(config, keypoints, descriptors, image)
    if result is None:
        return
    refImagePoints, imagePoints = result
    refPoints3d = unproject_image_points(refImagePoints, camera, refDistance)
    ...
    #                 [0,  0,  1]])
    K = camera
    a, t = solve_pnp(refPoints3d, imagePoints, K)
    logging.info('参考相机位置和角度：%s, %s', refPos.ravel(), refAzimuth)
    logging.info('相对参考照片的位置: %s', -t.A.ravel())
    logging.info('相对参考照片的偏转: %s', a * 180 / np.pi)

    # 确保方位角在正负180度之间
    azimuth = a - refAzimuth * np.pi / 180
    if azimuth > np.pi:
        azimuth -= np.pi * 2
    elif azimuth < -np.pi:
        azimuth += np.pi * 2

    # 这是返回的是相机坐标系，也就是Y轴朝下，Z 轴朝前，因为参考相机对于
    # 建筑物坐标系的正北方向是有一定角度，需要把相机的正前方转向正北，
    # 这样的偏移量才是相对建筑物坐标系
    tvec = rotation_matrix(refAzimuth * np.pi / 180, axis='y') * t.reshape(3, -1)
    return float(-azimuth * 180 / np.pi), refPos - tvec.A.ravel()
