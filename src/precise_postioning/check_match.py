...
def explore_match(config, kp_pairs, status = None, H = None):
    ...
    n0 = len(set(matched))
    logging.info('匹配的结果(unique/inlier/outlier)： %s / %s / %s', n0, n, len(kp_pairs))
    draw_str(vis, (100, 200), 'unique / inliner / outliner: %s / %s / %s' % (n0, n, len(kp_pairs)))

    if config.save:
        filename = '%s-%s.jpg' % (os.path.basename(config.filenames[0]).rsplit('.')[0], os.path.basename(config.filenames[1]).rsplit('.')[0])
        if config.output:
        ...
        with open(filename, 'w') as f:
            f.write('%-10s %-10s %-10s %-10s %-10s\n' % (os.path.basename(config.filenames[0]), os.path.basename(config.filenames[1]), n0, n, len(kp_pairs)))
