...
if nargin == 4    % nargin means number of input arguments
    precomputed = 1;
...
if isnumeric(D)    % 如果D是数值数组
    % [gist, param] = LMgist(img, HOMEIMAGES, param);
    Nscenes = size(D,4);
    typeD = 3;
    % fprintf('D is numeric\n');
...
for n = 1:Nscenes
    ...
    % if precomputed == 1    % when nargin == 4, it's true
    ...
    if todo == 1
        ...
        if precomputed
            mkdir(fullfile(HOMEGIST))
            % mkdir(fullfile(HOMEGIST, D(n).annotation.folder))
            save (fullfile(HOMEGIST, 'filegist.txt'), 'g', '-ascii')
        end
    end
